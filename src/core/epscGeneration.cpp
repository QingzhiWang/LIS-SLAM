  ...
  for (int i = 0; i < desc.rows; ++i) {
    for (int j = 0; j < desc.cols; ++j) {
      out.at<cv::Vec3b>(i, j)[0] =
          std::get<2>(Argmax2RGB[(int)desc.at<uchar>(i, j)]);
      out.at<cv::Vec3b>(i, j)[1] =
          std::get<1>(Argmax2RGB[(int)desc.at<uchar>(i, j)]);
      out.at<cv::Vec3b>(i, j)[2] =
          std::get<0>(Argmax2RGB[(int)desc.at<uchar>(i, j)]);
    }
  }
  return out;
}

pcl::PointCloud<pcl::PointXYZRGB>::Ptr EPSCGeneration::getColorCloud(
...
cv::Mat EPSCGeneration::project(
    pcl::PointCloud<PointXYZIL>::Ptr filtered_pointcloud) {
  ...
  for (uint i = 0; i < filtered_pointcloud->points.size(); i++) {
    ...
    if (label == 13 || label == 14 || label == 16 || label == 18 ||
        label == 19) {
      ...
      // if(ssc_dis.at<cv::Vec4f>(0,
      // sector_id)[3]<10||distance<ssc_dis.at<cv::Vec4f>(0, sector_id)[0]){
...
void EPSCGeneration::globalICP(cv::Mat &ssc_dis1, cv::Mat &ssc_dis2,
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    diff_y += dy;
    if (show) {
      std::cout << i << " diff " << diff_x << " " << diff_y << " " << dx << " "
                << dy << std::endl;
    }
...
Eigen::Matrix4f EPSCGeneration::globalICP(cv::Mat &ssc_dis1,
                                          cv::Mat &ssc_dis2) {
  double similarity = 100000;
  float angle = 0;
  int sectors = ssc_dis1.cols;
  ...
  for (int i = 0; i < sectors; ++i) {
    if (ssc_dis1.at<cv::Vec4f>(0, i)[3] > 0) {
      cloud1->push_back(pcl::PointXYZ(ssc_dis1.at<cv::Vec4f>(0, i)[1],
                                      ssc_dis1.at<cv::Vec4f>(0, i)[2], 0.));
    }
...
cv::Mat EPSCGeneration::calculateISC(
    const pcl::PointCloud<PointXYZIL>::Ptr filtered_pointcloud) {
...
cv::Mat EPSCGeneration::calculateSEPSC(
    const pcl::PointCloud<PointXYZIL>::Ptr filtered_pointcloud) {
  cv::Mat psc = cv::Mat::zeros(cv::Size(sectors, rings), CV_8U);
  cv::Mat esc = cv::Mat::zeros(cv::Size(sectors, rings), CV_8U);
  cv::Mat epsc = cv::Mat::zeros(cv::Size(sectors, rings), CV_8U);
...
cv::Mat EPSCGeneration::calculateSSC(
    const pcl::PointCloud<PointXYZIL>::Ptr filtered_pointcloud) {
  ...
  for (int i = 0; i < (int)filtered_pointcloud->points.size(); i++) {
    ...
    if (order_vec[label] > 0) {
      ...
      if (sector_id >= sectors || sector_id < 0) continue;
      if (order_vec[label] >
          order_vec[ssc.at<unsigned char>(ring_id, sector_id)]) {
...
double EPSCGeneration::calculateLabelSim(cv::Mat &desc1, cv::Mat &desc2) {
  ...
  for (int p = 0; p < sectors; p++) {
    for (int q = 0; q < rings; q++) {
      if (desc1.at<unsigned char>(q, p) == 0 &&
          desc2.at<unsigned char>(q, p) == 0) {
        continue;
      }
      valid_num++;
...
void EPSCGeneration::loopDetection(
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr pc_filtered_surf(
      new pcl::PointCloud<pcl::PointXYZI>());
  pcl::PointCloud<PointXYZIL>::Ptr pc_filtered_semantic(
  ...
  for (int i = 0; i < (int)posArr.size(); i++) {
    double delta_travel_distance =
        travelDistanceArr.back() - travelDistanceArr[i];
    double pos_distance =
        std::sqrt((posArr[i] - posArr.back()).array().square().sum());
  ...
  if (UsingISCFlag) {
    ...
    if (best_matched_id_isc != -1) {
      ...
      ROS_WARN(
          "ISC: received loop closure candidate: current: %d, history: %d, "
          "total_score: %f",
  ...
  if (UsingSCFlag) {
    ...
    if (best_matched_id_sc != -1) {
      ...
      ROS_WARN(
          "SC: received loop closure candidate: current: %d, history: %d, "
          "total_score: %f",
  ...
  if (UsingEPSCFlag) {
    ...
    if (best_matched_id_epsc != -1) {
      ...
      ROS_WARN(
          "EPSC: received loop closure candidate: current: %d, history: %d, "
          "total_score: %f",
  ...
  if (UsingSEPSCFlag) {
    ...
    if (best_matched_id_sepsc != -1) {
      ...
      ROS_WARN(
          "SEPSC: received loop closure candidate: current: %d, history: %d, "
          "total_score: %f",
  ...
  if (UsingSSCFlag) {
    ...
    if (best_matched_id_ssc != -1) {
      ...
      ROS_WARN(
          "SSC: received loop closure candidate: current: %d, history: %d, "
          "total_score: %f",
  ...
  if (UsingPoseFlag) {
    if (best_matched_id_pose != -1) {
      ...
      ROS_WARN(
          "POSE: received loop closure candidate: current: %d, history: %d, "
          "min_ditance: %f",
