...
public:
    ...
    ros::Publisher pubLoopContainer;
    ...
    loopClosure()
    {
        ...
        pubLoopContainer = nh.advertise<lis_slam::loop_container>("lio_loop/loop_container", 1);
 ...
 //---------------------------------
    //model 5    only  index
    //---------------------------------
    void performLoopClosureModelFive()
    {
        *copySubMapPose3D = *subMapPose3D;
        *copySubMapPose6D = *subMapPose6D;
        ...
        lis_slam::loop_container loopcontainer;

        sort(loopKeyPre.begin(),loopKeyPre.end());

        loopcontainer.header.stamp=timeLaserInfoStamp;
        loopcontainer.loopKeyCur =  (int)subMapPose3D->points[loopKeyCur].intensity;
        ...
        for(int i=0;i<loopKeyPre.size();i++){
            ...
            if(i>0 && id==loopKeyPre[i-1])
                continue;

            loopcontainer.loopKeyPre.push_back(id);

        }

        pubLoopContainer.publish(loopcontainer);
 ...
 //---------------------------------
    ...
    void performLoopClosureModelFour()
    {
        ...
        *cureKeyframeCloud+=*transformPointCloud(surfCloudSubMap[loopKeyCur], &copySubMapPose6D->points[loopKeyCur]);
        ...
        loopIndexQueue.push_back(make_pair(loopKeyCur, bestMatched));
        loopPoseQueue.push_back(correctionLidarFrame);
        // loopNoiseQueue.push_back(bestScore*0.01);//0.05
        loopNoiseQueue.push_back(0.1);//0.05
    ...
    bool detectLoopClosureDistanceModelFour(int *latestID, std::vector<int> *closestID)
    {
        ...
        ROS_INFO("EPSC: loopKeyCur: %d",loopKeyCur);
