...
class TransformFusion : public ParamServer {
 public:
  ...
  TransformFusion() {
    // subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("odometry/fusion", 5,
    // &TransformFusion::lidarOdometryHandler, this,
    // ros::TransportHints().tcpNoDelay());
    subLaserOdometry = nh.subscribe<nav_msgs::Odometry>(
        "lis_slam/submap/odometry", 5, &TransformFusion::lidarOdometryHandler,
  ...
  void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg) {
    ...
    if (imuTime - last_path_time > 0.1) {
      ...
      while (!imuPath.poses.empty() &&
             imuPath.poses.front().header.stamp.toSec() < lidarOdomTime - 0.1)
 ...
 public:
  ...
  IMUPreintegration() {
    subImu = nh.subscribe<sensor_msgs::Imu>(
        ...
        ros::TransportHints().tcpNoDelay());
    // subOdometry =
    // nh.subscribe<nav_msgs::Odometry>("odometry/fusion_incremental", 5,
    // &IMUPreintegration::odometryHandler, this,
    // ros::TransportHints().tcpNoDelay());
    subOdometry = nh.subscribe<nav_msgs::Odometry>(
        "lis_slam/submap/odometry_incremental", 5,
        &IMUPreintegration::odometryHandler, this,
        ros::TransportHints().tcpNoDelay());
    ...
    // << 1e-1, 1e-1, 1e-1, 1.0, 1.0, 1.0).finished()); // rad,rad,rad,m, m, m
    priorVelNoise = gtsam::noiseModel::Isotropic::Sigma(3, 1e4);  // m/s
    priorBiasNoise = gtsam::noiseModel::Isotropic::Sigma(
        6, 1e-3);  // 1e-2 ~ 1e-3 seems to be good
    // priorBiasNoise  = gtsam::noiseModel::Isotropic::Sigma(6, 1e-2); // 1e-2 ~
    // 1e-3 seems to be good
