        ...
        while (ros::ok()) {
			...
			if(!keyFrameQueue.empty())
			{
        		...
        		if (thisCurId != keyFrameInfo.end()){
					...
					ROS_WARN("loopClosureThread -->> Dont find keyFrameInfo[%d]!", curKeyFrame);
					continue;
				}
...
class SubMapOptmizationNode : public SubMapManager<PointXYZIL> {
 public:
    ...
    void publishGlobalMap()
    {
        ...
        publishCloud(&pubCloudMap, globalMapCloud, timeSubMapInfoStamp, mapFrame);
    }
    ...
    void subMapOptmizationThread() 
	{
		...
		{	
			...
			if(!subMapIndexQueue.empty())
			{
				/* code */
				auto t1 = ros::Time::now();
        		std::lock_guard<std::mutex> lock(subMapMtx);
        		...
        		if (thisCurId != subMapInfo.end()){
        			curSubMapPtr = subMapInfo[curSubMapId];
					if(curSubMapId != curSubMapPtr->submap_id)
					{
                ...
                timeSubMapInfoStamp = cloudInfo.header.stamp;
    ...
    void extractSubMapCloud()
    {
		...
		Eigen::Affine3f auto thisCurId = subMapInfo.find(preSubMapId);
		if (thisCurId != subMapInfo.end()){
			...
			*laserCloudSurfFromSubMap = *transformPointCloud(laserCloudSurfFromSubMap, &preSubMap->submap_pose_6D_optimized);
			...
			laserCloudCornerLast->points.insert(laserCloudCornerLast->points.end(), 
												currentKeyFrame->submap_pole->points.begin(), 
												currentKeyFrame->submap_pole->points.end());
			laserCloudSurfLast->points.insert(laserCloudSurfLastDS->points.end(), 
												currentKeyFrame->submap_dynamic->points.begin(), 
												currentKeyFrame->submap_dynamic->points.end());
			laserCloudSurfLast->points.insert(laserCloudSurfLastDS->points.end(), 
												currentKeyFrame->submap_ground->points.begin(), 
												currentKeyFrame->submap_ground->points.end());
			laserCloudSurfLast->points.insert(laserCloudSurfLastDS->points.end(), 
												currentKeyFrame->submap_building->points.begin(), 
												currentKeyFrame->submap_building->points.end());
    ...
    void addOdomFactor()
    {
		...
		{
			int indexTo = subMapIndexContainer[i].first;
			int indexFrom = subMapIndexContainer[i].second;
		...
		int indexTo = subMapIndexContainer[0].first;
		gtsam::Pose3 poseTo   = trans2gtsamPose(transformTobeMapped);
		initialEstimate.insert(indexTo, poseTo);
    ...
    void correctPoses()
    {
        ...
        if (aLoopIsClosed == true)
        {
            ...
            for (int i = 0; i < numPoses; ++i)
            {
        		...
        		if (thisCurId != subMapInfo.end()){
        			subMapInfo[curSubMapId]->submap_pose_6D_optimized = cloudSubMapPoses6D->points[i];
        			subMapInfo[curSubMapId]->submap_pose_3D_optimized = cloudSubMapPoses3D->points[i];
				}else{
    ...
    void scan2SubMapOptimization()
    {
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
			...
			subMapIndexContainerAll.push_back(make_pair(curSubMapId, preSubMapId));
			subMapIndexContainer.push_back(make_pair(curSubMapId, preSubMapId));
