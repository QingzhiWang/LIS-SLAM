...
public:
    ...
    FeatureExtraction()
    {
        subLaserCloudInfo = nh.subscribe<lis_slam::cloud_info>("lis_slam/deskew/cloud_info", 1, &FeatureExtraction::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());

        pubLaserCloudInfo = nh.advertise<lis_slam::cloud_info> ("lis_slam/feature/cloud_info", 1);
        pubCornerPoints = nh.advertise<sensor_msgs::PointCloud2>("lis_slam/feature/cloud_corner", 1);
        pubSurfacePoints = nh.advertise<sensor_msgs::PointCloud2>("lis_slam/feature/cloud_surface", 1);
    ...
    void publishFeatureCloud()
    {
        ...
        cloudInfo.cloud_corner  = publishRawCloud(&pubCornerPoints,  cornerCloud,  cloudHeader.stamp, lidarFrame);
        cloudInfo.cloud_surface = publishRawCloud(&pubSurfacePoints, surfaceCloud, cloudHeader.stamp, lidarFrame);
