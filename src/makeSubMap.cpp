...
class makeSubMap : public ParamServer
{
...
public:
    ...
    ros::Publisher pubSubMapRaw;
    ros::Publisher pubSubMapCorner;
    ros::Publisher pubSubMapSurf;
    ros::Publisher pubLaserOdometryIncremental;
    ros::Publisher pubLaserOdometryGlobal;

    ros::Publisher pubSubMapId;

    ros::Publisher pubSubMapRawTest;

    lis_slam::cloud_info cloudInfo;
    lis_slam::submap submapInfo;

    lis_slam::submap  mapInfo;
    ...
    pcl::PointCloud<PointType>::Ptr laserCloudCornerFromSubMap;
    pcl::PointCloud<PointType>::Ptr laserCloudSurfFromSubMap;
    pcl::PointCloud<PointType>::Ptr laserCloudCornerFromSubMapDS;
    pcl::PointCloud<PointType>::Ptr laserCloudSurfFromSubMapDS;

    pcl::PointCloud<PointType>::Ptr laserCloudCornerFromPreSubMap;
    pcl::PointCloud<PointType>::Ptr laserCloudSurfFromPreSubMap;
    pcl::PointCloud<PointType>::Ptr laserCloudCornerFromPreSubMapDS;
    pcl::PointCloud<PointType>::Ptr laserCloudSurfFromPreSubMapDS;


    pcl::PointCloud<PointType>::Ptr laserCloudRawLast; 
    pcl::PointCloud<PointType>::Ptr laserCloudRawLastDS; 
    pcl::PointCloud<PointType>::Ptr laserCloudRawFromSubMap; 
    pcl::PointCloud<PointType>::Ptr laserCloudRawFromSubMapDS;
    ...
    pcl::KdTreeFLANN<PointType>::Ptr kdtreeCornerFromSubMap;
    pcl::KdTreeFLANN<PointType>::Ptr kdtreeSurfFromSubMap;
    ...
    makeSubMap() 
    {
        subCloudInfo = nh.subscribe<lis_slam::cloud_info>("lis_slam/feature/cloud_info", 1, &makeSubMap::laserCloudInfoHandler, this);
        // subMapInfo =  nh.subscribe<lis_slam::submap>("lis_slam/mapping/submap_info", 1, &makeSubMap::mapInfoHandler, this);

        pubSubMapInfo = nh.advertise<lis_slam::submap>("lis_slam/submap/submap_info", 1);
        pubSubMapRaw = nh.advertise<sensor_msgs::PointCloud2>("lis_slam/submap/submap_raw", 1);
        pubSubMapCorner = nh.advertise<sensor_msgs::PointCloud2>("lis_slam/submap/submap_corner", 1);
        pubSubMapSurf = nh.advertise<sensor_msgs::PointCloud2>("lis_slam/submap/submap_surf", 1);

        pubLaserOdometryGlobal      = nh.advertise<nav_msgs::Odometry> ("lis_slam/submap/odometry", 1);
        pubLaserOdometryIncremental = nh.advertise<nav_msgs::Odometry> ("lis_slam/submap/odometry_incremental", 1);
       
         pubSubMapId = nh.advertise<sensor_msgs::PointCloud2>("lis_slam/submap/submap_id", 1);

        pubSubMapRawTest= nh.advertise<sensor_msgs::PointCloud2>("lis_slam/submap/submap_raw_test", 1);
    ...
    void allocateMemory()
    {
        ...
        laserCloudCornerFromPreSubMap.reset(new pcl::PointCloud<PointType>());
        laserCloudSurfFromPreSubMap.reset(new pcl::PointCloud<PointType>());
        laserCloudCornerFromPreSubMapDS.reset(new pcl::PointCloud<PointType>());
        laserCloudSurfFromPreSubMapDS.reset(new pcl::PointCloud<PointType>());

        laserCloudRawLast.reset(new pcl::PointCloud<PointType>()); 
        laserCloudRawLastDS.reset(new pcl::PointCloud<PointType>()); 
        laserCloudRawFromSubMap.reset(new pcl::PointCloud<PointType>()); 
        laserCloudRawFromSubMapDS.reset(new pcl::PointCloud<PointType>()); 
    ...
    void mapInfoHandler(const lis_slam::submapConstPtr& msgIn)
    {
        ...
        if(mapInfo.isOptmization)
        {
            afterMapOptmizationPoses[3]=mapInfo.afterOptmizationSubMapPoseX;
            afterMapOptmizationPoses[4]=mapInfo.afterOptmizationSubMapPoseY;
            afterMapOptmizationPoses[5]=mapInfo.afterOptmizationSubMapPoseZ;
            afterMapOptmizationPoses[0]=mapInfo.afterOptmizationSubMapPoseRoll;
            afterMapOptmizationPoses[1]=mapInfo.afterOptmizationSubMapPosePitch;
            afterMapOptmizationPoses[2]=mapInfo.afterOptmizationSubMapPoseYaw;
            
            if(mapInfo.subMapId!=0)
            {               
                //***********************************
                //纠正位姿
                //***********************************
                int preId=mapInfo.subMapId;
                Eigen::Affine3f transBeforOptmization = pclPointToAffine3f(subMapPose6D->points[preId]);
                // Eigen::Affine3f transAfterOptmization = pcl::getTransformation(mapInfo.subMapPoseX,    mapInfo.subMapPoseY,     mapInfo.subMapPoseZ, 
                //                                                 mapInfo.subMapPoseRoll, mapInfo.subMapPosePitch, mapInfo.subMapPoseYaw);
                Eigen::Affine3f transAfterOptmization = trans2Affine3f(afterMapOptmizationPoses);
                
                transDelta = transAfterOptmization * transBeforOptmization.inverse();

                isMapOptmization=true;
            }
    ...
    void makeSubMapThread(){
        ...
        while(ros::ok()){
            ...
            static ros::Time submapStartTime=ros::Time::now();
            ...
            if(curSubMapSize>=(int)(subMapFramesSize/3))
            {
                ...
                float makeSubMapTime=(submapEndTime - submapStartTime).toSec();
                // if(subMapYawSum>=subMapYawMax||curSubMapSize>=subMapFramesSize)
                if(subMapYawSum>=subMapYawMax||curSubMapSize>=subMapFramesSize||makeSubMapTime>=subMapMaxTime)
                {
                    subMapYawSum=0;
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO("\033[1;32m----> Make SubMap Started.\033[0m");

    std::thread submapthread(&makeSubMap::makeSubMapThread, &MS);
